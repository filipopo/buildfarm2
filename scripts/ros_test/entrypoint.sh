...
if [[ "$FAILED" -eq 0 ]]; then
  ...
  lcov --capture --directory ${WORKSPACE} --output-file /tmp/coverage.original --ignore-errors mismatch,mismatch,gcov,gcov,negative,negative || echo "$0: no coverage data to extract"
